...
def iousd(atlbrs, btlbrs):
    ...
    ious = bbox_diou(
        torch.from_numpy(np.ascontiguousarray(atlbrs, dtype=np.float)),
        torch.from_numpy(np.ascontiguousarray(btlbrs, dtype=np.float))
    )

    return ious


def bbox_overlaps_diounew(bboxes1, bboxes2):
    ...
    # dious = inter_area / union * 0.85 + 0.15 * (1 - (inter_diag) / outer_diag)
...
def bbox_diou(bboxes1, bboxes2):
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            #
            # center_x1 = (bboxes1[i][2] + bboxes1[i][0]) / 2
            # # center_y1 = (bboxes1[i][3] + bboxes1[i][1]) / 2
            # center_x2 = (bboxes2[j][2] + bboxes2[j][0]) / 2
            # # center_y2 = (bboxes2[j][3] + bboxes2[j][1]) / 2
            #
            # bboxes1[i][0] = center_x1 - w1 * 0.65
            # # bboxes1[i][1] = center_y1 - h1 * 0.65
            # bboxes1[i][2] = center_x1 + w1 * 0.65
            # # bboxes1[i][3] = center_y1 + h1 * 0.65
            #
            # bboxes2[j][0] = center_x2 - w2 * 0.65
            # # bboxes2[j][1] = center_y2 - h2 * 0.65
            # bboxes2[j][2] = center_x2 + w2 * 0.65
            # # bboxes2[j][3] = center_y2 + h2 * 0.65

            dious[i, j] = bbox_overlaps_diounew(bboxes1[i], bboxes2[j])
    if dious.ndim == 1:
        dious = torch.unsqueeze(dious, 1)
    dious = dious.numpy()
    return dious

def iou_distanced(atracks, btracks):
    """
    Compute cost based on IoU
    :type atracks: list[STrack]
    :type btracks: list[STrack]
