...
from  tracker.hqg_tracker import BoTSORT
        ...
        for cur_iter, batch_data in enumerate(
            ...
            with torch.no_grad():
                ...
                if frame_id == 1:
                    if self.args.track.byte:
                        ...
                        tracker = SparseTracker(self.args.track, videoname=video_name)
                    elif self.args.track.bot:
                        tracker = BoTSORT(self.args.track, videoname=video_name)
                    elif self.args.track.sort:
            ...
            if det_instances is not None:
                online_targets = tracker.update(
                    det_instances, batch_data[0]["ori_img"], path=img_file_name
                )
                online_tlwhs = []
                online_ids = []
                online_scores = []
